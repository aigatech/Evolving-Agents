...
from src.utils.trt_backbone import TRTHttpVisionBackbone
...
def get_prismatic_vla(cfg):
    ...
    # 2) Vision backbone
    # Use TRT HTTP service only when explicitly configured. Otherwise keep the original backbone.
    trt_base_url = cfg.trt_vision_engine or os.getenv("TRTVISION_BASE_URL")
    require_trt_cfg = getattr(cfg, "require_trt_vision", False)
    if not require_trt_cfg:
        os.environ.setdefault("REQUIRE_TRT", "0")  # allow graceful fallback unless explicitly required
    env_requires_trt = os.environ.get("REQUIRE_TRT", "0") not in {"0", "false", "False"}
    require_trt = require_trt_cfg or env_requires_trt

    if trt_base_url:
        print(f"[*] Attempting to use TRT vision backend at {trt_base_url}")
        try:
            trt_vis = TRTHttpVisionBackbone.from_existing(
                vla.vision_backbone,
                base_url=trt_base_url,
                out_num_patches=256,
                out_embed_dim=2176,
            )
            ...
            if require_trt:
                raise
            print(f"[WARN] Falling back to PyTorch vision backbone because TRT backend is unavailable: {err}")
        else:
            vla.vision_backbone = trt_vis
            print("[DEBUG] replaced vision backbone:", type(vla.vision_backbone), vla.vision_backbone.identifier)
    else:
        print("[*] TRT vision backend not configured; using PyTorch vision backbone.")
